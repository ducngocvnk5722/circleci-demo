...
func (r *Render) Create() *Render {
	...
	for _, tpl := range append(tplRoot, tplSub...) {
		...
		tpls := append([]string{layout}, tpl)
		subDir := strings.TrimSuffix(tpl, r.Ext)
		tplDirFile, err := filepath.Glob(subDir + "/*" + r.Ext)
		if err != nil {
			panic(err.Error())
		}
		tpls = append(tpls, tplDirFile...)
		r.AddFromFiles(name, tpls...)
	}
